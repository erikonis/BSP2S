...
Picks SAMPLES_PER_CATEGORY number of files per category randomly, to be used as test set.
"""

# Hyper parameters
OUTPUT_DIR = "output"
HUMAN_DIR = "Java_20perEx_img_hum_preproc_pool"
AI_DIR = "Java_20perEx_img_ai_preproc_pool"
...
def organize_images(class0_dir, class1_dir, base_output_dir, samples_per_category=1000):
    ...
    # process_codes -> [item_name, image, image_origin, image_quality]
    ...
    for label, lst in label_files.items():
        ...
        if len(lst) >= samples_per_category:
            ...
            # lst -> [[item_name, image, image_origin, image_quality], ...]

            test_files = random.sample(lst, samples_per_category)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            with tqdm(total=len(test_files), desc="Moving test set files") as pbar:
                futures = [
                    executor.submit(
                        save_image,
                        lst[1],  # [img_name, img, img_origin, img_quality]

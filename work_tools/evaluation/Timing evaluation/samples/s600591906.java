...
//      System.err.println(Arrays.toString(sa));
      if(sa.length==1){
        a[i]=Long.valueOf(sa[0])*1_000_000_000L;
      }else{
        a[i]=Long.valueOf(sa[1])*repow(10,9-sa[1].length());
        a[i]=a[i]+(Long.valueOf(sa[0])*1_000_000_000L);
      }
    }
    int[][] tf=new int[19][19];
    for(int i=0;i<n;i++){
      int t=0;
      int f=0;
      while(a[i]%2==0){
...
//    System.err.println(Arrays.deepToString(tf));
    
    long ans=0;
    for(int ti=0;ti<19;ti++){
      for(int fi=0;fi<19;fi++){
        long ai=0;
        for(int to=18-ti;to<19;to++){
          for(int fo=18-fi;fo<19;fo++){
            if(ti==to&&fi==fo){
              ans=ans+1L*tf[to][fo]*(tf[ti][fi]-1);
            }else{
              ans=ans+1L*tf[to][fo]*tf[ti][fi];
            }
          }
        }
      }

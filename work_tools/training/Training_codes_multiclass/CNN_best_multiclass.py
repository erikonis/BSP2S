...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Validation - Loss: {val_epoch_loss:.4f} - Accuracy: {val_epoch_acc:.2f}%"
        )
...
def evaluate_model(
    ...
    try:
        roc_auc = roc_auc_score(test_labels, test_probs, multi_class="ovr")
    except Exception:
        roc_auc = float("nan")
    ...
    plt.tick_params(axis="both", which="major", pad=25, labelsize=16)
    plt.title(f"{full_model_name}\nAccuracy {formatted_accuracy}%", pad=20, fontsize=20)
    plt.tight_layout()
    ...
    # Save confidence statistics for all classes
    save_confidence_statistics(
        test_probs, test_labels, test_preds, model_name, output_dir, labels
    )


def save_confidence_statistics(
    test_probs, test_labels, test_preds, model_name: str, output_dir: str, labels: list
):
    """Save confidence statistics in a formatted table for each class"""
    header = f"{'Model':<22} {'Label':<16} {'Samples':<8} {'Average Confidence':<18} {'Max Confidence':<14} {'Min Confidence':<14}"
    separator = "-" * len(header)
    lines = []
    stats_per_class = []
    for idx, label in enumerate(labels):
        stats = get_class_confidence_stats(
            test_probs, test_labels, test_preds, class_label=idx
        )
        stats_per_class.append(stats)
        line = f"{model_name:<22} {label:<16} {stats['samples']:<8} {stats['avg_confidence']:<18.3f} {stats['max_confidence']:<14.3f} {stats['min_confidence']:<14.3f}"
        lines.append(line)
    ...
    with open(conf_stats_path, "a") as f:
        ...
        for line in lines:
            f.write(line + "\n")
        f.write(
            "\nTable: Confidence Score Statistics for model acting as oracle on the accurately sorted test codes\n"
        )
...
def get_class_confidence_stats(probs, labels, pred_labels, class_label):
    """Calculate confidence statistics for correctly classified samples of a given class (multiclass)"""
    # probs: shape (N, num_classes), labels: (N,), pred_labels: (N,)
    correct_mask = (labels == class_label) & (pred_labels == labels)
    if np.sum(correct_mask) == 0:
        return {
            "samples": 0,
            "avg_confidence": float(np.nan),
            "max_confidence": float(np.nan),
            "min_confidence": float(np.nan),
        }
    class_probs = probs[correct_mask, class_label]
    return {
...
def training_cycle(
    ...
    # Adjust model output layer if needed (user must ensure model matches num_classes)
    ...
    evaluate_model(
        model, test_loader, device, output, model_name, full_model_name, labels
    )
